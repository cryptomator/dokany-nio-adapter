...
public class MemoryFS extends DokanyFileSystem {
	...
	private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(MemoryFS.class);
	/**
	 * Path matcher glob
	 */
	...
	public MemoryFS(@NonNull final DeviceOptions deviceOptions, @NonNull final VolumeInformation volumeInfo, @NonNull final FreeSpace freeSpace, @NonNull final Date rootCreationDate, @NonNull final String rootPath) throws IOException {
		super(deviceOptions, volumeInfo, freeSpace, rootCreationDate, rootPath);
		if (deviceOptions == null) {
			throw new NullPointerException("deviceOptions");
		}
		if (volumeInfo == null) {
			throw new NullPointerException("volumeInfo");
		}
		if (freeSpace == null) {
			throw new NullPointerException("freeSpace");
		}
		if (rootCreationDate == null) {
			throw new NullPointerException("rootCreationDate");
		}
		if (rootPath == null) {
			throw new NullPointerException("rootPath");
		}
		// Try to create store location in temp directory
		final java.nio.file.Path fileStorePath = Files.createTempDirectory("dokany-java_");
		// Init VFS
	...
	private File getExistingFile(@NonNull final String path, @NonNull final Transaction txn) {
		...
		if (txn == null) {
			throw new NullPointerException("txn");
		}
	...
	private File getExistingFileWithException(@NonNull final String path, @NonNull final Transaction txn) throws FileNotFoundException {
		...
		final Object file = getExistingFile(path, txn);
	...
	*/
	...
	public Set<WIN32_FIND_DATA> findFilesWithPattern(@NonNull final String pathToSearch, @NonNull final DokanyFileInfo dokanyFileInfo, @NonNull final String pattern) {
		if (pathToSearch == null) {
			throw new NullPointerException("pathToSearch");
		}
		if (dokanyFileInfo == null) {
			throw new NullPointerException("dokanyFileInfo");
		}
		if (pattern == null) {
			throw new NullPointerException("pattern");
		}
		final Object files = new HashSet<WIN32_FIND_DATA>();
		log.debug("findFilesWithPattern memoryfs:   path  {};     pattern {}", pathToSearch, pattern);
		// Only use if pattern is not null
		final java.nio.file.PathMatcher pathMatcher = DEFAULT_FS.getPathMatcher(GLOB + root + pattern);
		env.executeInReadonlyTransaction((@NonNull final Transaction txn) -> {
			vfs.getFiles(txn).forEach(file -> {
				final Object currentPath = file.getPath();
	...
	public void move(@NonNull final String oldPath, @NonNull final String newPath, final boolean replaceIfExisting) throws IOException {
		if (oldPath == null) {
			throw new NullPointerException("oldPath");
		}
		if (newPath == null) {
	...
	public FullFileInfo getInfo(@NonNull final String path) throws IOException {
		...
		final Object result = env.computeInReadonlyTransaction((@NonNull final Transaction txn) -> {
	...
	private FullFileInfo getInfo(@NonNull final String path, @NonNull final Transaction txn) throws FileNotFoundException {
		...
		final Object pathKey = StringBinding.stringToEntry(path);
		final Object iterable = infoStore.get(txn, pathKey);
	 ...
	 * @throws FileNotFoundException
	 */
	private void createEmptyFile(@NonNull final String path, final long options, final EnumIntegerSet<FileAttribute> attributes, @NonNull final Transaction txn) throws FileNotFoundException {
		if (path == null) {
			throw new NullPointerException("path");
		}
		if (txn == null) {
		...
		final Object file = createFile(path, txn);
	...
	public FileData read(@NonNull final String path, final int offset, final int readLength) throws IOException {
		...
		final Object fileData = env.computeInTransaction((@NonNull final Transaction txn) -> {
			FileData toReturn = null;
			final Object file = getExistingFile(path, txn);
			final Object fileSize = vfs.getFileLength(txn, file);
			if (fileSize > 0) {
				if (Objects.nonNull(file)) {
					final VfsInputStream inputStream = vfs.readFile(txn, file);
					try {
						final byte[] data = new byte[readLength];
						final Object numRead = inputStream.read(data, offset, Math.min(readLength, data.length - offset));
	...
	public int write(@NonNull final String path, final int offset, final byte[] data, final int writeLength) throws IOException {
		...
		final Object error = env.computeInTransaction((@NonNull final Transaction txn) -> {
			IOException toReturn = null;
			try {
				final Object file = getExistingFileWithException(path, txn);
	...
	private int write(@NonNull final File file, final int offset, final byte[] data, final int writeLength, final FullFileInfo info, @NonNull final Transaction txn) throws IOException {
		if (file == null) {
			throw new NullPointerException("file");
		}
		if (txn == null) {
	...
	@Override
	public void setTime(@NonNull final String path, @NonNull final FILETIME creation, @NonNull final FILETIME lastAccess, @NonNull final FILETIME lastModification) throws IOException {
		if (path == null) {
			throw new NullPointerException("path");
		}
		if (creation == null) {
			throw new NullPointerException("creation");
		}
		if (lastAccess == null) {
			throw new NullPointerException("lastAccess");
		}
		if (lastModification == null) {
			throw new NullPointerException("lastModification");
		}
		final FullFileInfo info = getInfo(path);
	...
	private void createSampleItems() throws IOException {
		final Object error = env.computeInTransaction((@NonNull final Transaction txn) -> {
			...
			try {
				...
				final FullFileInfo info = new FullFileInfo(root, file.getDescriptor(), attributes, volumeInfo.getSerialNumber());
